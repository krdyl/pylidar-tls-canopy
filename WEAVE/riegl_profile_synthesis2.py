...
file_data_rdb = file_data[file_data["File_type"] == "allrdbs"]

file_data_rxp = file_data[file_data["File_type"] == "allrxps"]
...
def get_plantprofiles(rxp, rdb, country, project, scan):
    try:
        ...
        x,y,z,r = plant_profile_2.get_min_z_grid([upright_rxp_fn], 
        ...
        vpp = plant_profile_2.Jupp2009(hres=0.5, zres=5, ares=90, 
        ...
        name = "/home/kdayal/projects/pylidar-tls-canopy/results/synthesis/new2/" + country +"_"+ project + "_" + scan + ".csv"
        df.to_csv(name)
...
def process_row(row):
    rxp, rdb = row  # tuple passed in
    p = Path(rxp)
    # safer path parsing
    parts = p.parts
    country = parts[-4] if len(parts) >= 4 else ""   # adjust to your layout
...
with ThreadPoolExecutor(max_workers=num_cores) as executor:
    futures = [executor.submit(process_row, (row.Full_path_x, row.Full_path_y))
               for row in file_locs.itertuples(index=False)]
...
print(sum(r.startswith("Processed") for r in results), "ok |",
      sum(r.startswith("Error") for r in results), "errors")
